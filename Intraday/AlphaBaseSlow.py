...
class Intraday:
    ...
    def update(self):
        ...
        try:
            self.pricePast = np.load("/mnt/data/data_cache/pastPrice/{}.npy".format(self.now))
            self.volumePast = np.load("/mnt/data/data_cache/pastVolume/{}.npy".format(self.now))
            
        except FileNotFoundError:
            self.pricePast = np.nan_to_num(np.stack(list(df.reindex(self.price.columns, axis=1) for df in self.recorderPrice)))
            self.volumePast = np.nan_to_num(np.stack(list(df.reindex(self.volume.columns, axis=1) for df in self.recorderVolume)))
            np.save("/mnt/data/data_cache/pastPrice/{}.npy".format(self.now), self.pricePast)
            np.save("/mnt/data/data_cache/pastVolume/{}.npy".format(self.now), self.volumePast)
            print("No cache available, use raw data instead ...")
    ...
    def allocate(self):
        ...
        # print("{}/{}".format((long_stock>0).sum(),(short_stock>0).sum()))
