...
from KatePublic.DataAPI.db_tools import *
...
class Intraday:
    def __init__(self, sim_start, sim_end, verbose=1, log=True):
        ...
        self.sim_period = [d for d in CALENDAR if sim_start <= d <= sim_end][::-1]
    ...
    def update(self):
        ...
        self.tradingStatusRaw = (
            tradingStatus if len(tradingStatus) else self.tradingStatusRaw
        )
        self.tradingStatus = self.tradingStatusRaw == "trade"

        self.pool = list(set(self.volumeRaw.columns) & set(self.tradingStatus.columns))
        ...
        if self.nextTradingDate:
            ...
            self.pool = list(set(self.nextOpenTWAP.index) & set(self.pool))
            ...
            self.limitFlag = (
                (self.nextPrice.iloc[-1, :] / self.price.iloc[-1, :] - 1).abs()
                + (self.nextPrice.iloc[2, :] / self.price.iloc[-1, :] - 1).abs()
            ).fillna(0) < 0.18
            ...
            self.nextReturn = (
                (self.nextOpenTWAP - self.openTWAP) / self.openTWAP
            ).fillna(0)
        ...
        self.prePool = list(set(self.signal.index) & set(self.pool))
    ...
    def crossSection(self, how="normal"):
        ...
        if how == "normal":
            self.preSignal = (
                (self.preSignal - self.preSignal.mean()) / self.preSignal.std()
            ).fillna(0)
        elif how == "rank":
    ...
    def timeSeris(self):
        ...
        df.fillna(df.mean())
        self.preSignal = ((df.iloc[:, -1] - df.mean(axis=1)) / df.std(axis=1)).loc[
            self.preSignal.index
        ]
    ...
    def allocate(self):
        ...
        """
        self.preSignal.to_csv("factor_values/" + self.now + ".csv")
        long_stock = (
            np.where(
                self.preSignal >= np.quantile(self.preSignal, 0.8),
                self.preSignal,
                -self.positionSmooth,
            )
            + self.positionSmooth
    ...
    def run(self, log=True):
        ...
        with open("./logs/log-{}.csv".format(self.log_time), "w") as output:
            print("date,long,short,excess return,culmulative return", file=output)
            while len(self.sim_period) > 1:
                ...
                long_part = (
                    self.weights["long"] * self.nextReturn
                ).sum() / self.weights["long"].sum()
                short_part = (
                    self.weights["short"] * self.nextReturn
                ...
                if self.verbose == 2:
                    print(
                        ...
                        self.now,
                        "  \tlong: {:7.4f}  \tshort: {:7.4f}  \texcess return: {:7.4f}  \tcumulative return: {:7.4f}".format(
                            long_part, short_part, excess_return, self.cum_return
                        ),
                    )
                else:
                    if (
                        self.now.split("-")[self.verbose]
                        != lastDate.split("-")[self.verbose]
                    ):
                        print(
                            "[{}]".format(datetime.datetime.now() - startTime),
                            lastDate,
                            "excess return: {:7.4f}  \tcumulative return: {:7.4f}".format(
        ...
        with open("./formulas/formula-{}.txt".format(self.log_time), "w") as output:
    ...
    def evaluate(cls, log_time):
        ...
        result["historyHigh"] = [
            max(result["culmulative return"][: i + 1]) for i in range(len(result))
        ]
        result["drawdown"] = 1 - result["culmulative return"] / result["historyHigh"]
        ...
        ax.plot(
            result["date"], result["short"].cumsum(), c="green", label="Short(index)"
        )
        ax.plot(
            result["date"],
            (result["long"] - result["short"]).cumsum(),
            c="blue",
            label="Hedged",
        )
        ...
        ax3 = fig.add_axes([0.18, 0.65, 0.2, 0.2])
        ax3.hist(
            result["excess return"], bins=50, density=False, edgecolor="b", alpha=0.6
        )
            ...
            pd.DataFrame(
                {
                    ...
                    * result.groupby("year")["excess return"].mean()
                    ...
                    "long": result.groupby("year")["long"].prod() - 1,
                    "short": result.groupby("year")["short"].prod() - 1,
                    "bp(hedged)": result.groupby("year")["excess return"].mean()
                    * 10000,
        ...
        benchmarks = [
            ...
            for f in os.listdir("./alphaResults")
            if f.endswith(".csv")
        ]
        ...
        if "SimFlag" not in os.listdir("./alphaResults"):
            ...
            for bm in benchmarks:
                ...
                corrRes[bm.split("/")[-1].split(".")[0]] = np.corrcoef(
                    riskFactor["excess return"], result["excess return"]
                )[0, 1]
            res = pd.DataFrame(corrRes, index=["corr"]).T
